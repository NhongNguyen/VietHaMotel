...
/**
    ...
    {
            // Tìm kiếm hóa đơn dựa trên ID
        $hoaDon = HoaDons::findOrFail($id);

        // Tìm kiếm chi tiết hóa đơn dựa trên ID hóa đơn
        $chiTietHoaDon = ChiTietHoaDons::where('id_hoa_don', $id)->firstOrFail();
        
        // Tạo một view PDF sử dụng dữ liệu từ hóa đơn và chi tiết hóa đơn
        $view = View::make('cthddownload', compact('hoaDon', 'chiTietHoaDon'));
