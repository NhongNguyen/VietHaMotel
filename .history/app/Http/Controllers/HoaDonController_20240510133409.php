...
class HoaDonController extends Controller
{
    ...
    public function autoCreateChiTiet(Request $request, $id)
    {
        // Lấy danh sách các dịch vụ được chọn từ request
        $selectedServices = $request->input('id_dich_vu');
    
        // Lấy hóa đơn dựa trên ID được chỉ định
        $hoaDon = HoaDons::findOrFail($id);
    
        // Kiểm tra xem hóa đơn có hợp đồng không
        if ($hoaDon->hopdongs) {
            // Kiểm tra xem đã tạo chi tiết hóa đơn dịch vụ (chitiethoadondichvu) cho hóa đơn này chưa
            $hasChiTietHoaDonDichVu = ChiTietHoaDonDichVus::where('id_chi_tiet_hoa_don', $hoaDon->id)->exists();
            
            // Nếu chưa tạo, chuyển hướng người dùng đến trang tạo chi tiết hóa đơn dịch vụ
            if (!$hasChiTietHoaDonDichVu) {
                return redirect()->route('create-chitiethoadondichvu', ['id' => $hoaDon->id]);
            }
            ...
            // Bạn có thể thay đổi cách tính này tùy thuộc vào logic của ứng dụng của bạn
            ...
            // Bạn có thể thay đổi cách lấy thông tin này tùy thuộc vào cấu trúc cơ sở dữ liệu của bạn
        ...
        return redirect('admin/hoadon')->with('success', 'Chi tiết hóa đơn đã được tạo tự động.');
    }
