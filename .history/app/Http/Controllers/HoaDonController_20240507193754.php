...
/**
    ...
    // Kiểm tra xem hóa đơn có tồn tại không trước khi truy cập thuộc tính khachThue và phongTro
    if ($hoaDon) {
        // Lấy thông tin khách hàng và phòng trọ từ hóa đơn nếu tồn tại
        $khachThue = $hoaDon->khachThue;
        $phongTro = $hoaDon->phongTro;

        // Tạo một view PDF sử dụng dữ liệu từ chi tiết hóa đơn, khách hàng và phòng trọ
        $view = View::make('cthddownload', compact('chiTietHoaDon', 'khachThue', 'phongTro'));

        // Tạo một đối tượng Dompdf và render PDF
        $dompdf = new Dompdf();
        ...
        // Trả về một thông báo hoặc xử lý lỗi khác nếu hóa đơn không tồn tại
        // Ví dụ: return redirect()->back()->withErrors(['message' => 'Hóa đơn không tồn tại.']);
    }
    }
