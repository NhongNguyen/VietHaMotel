...
function dashboard(){
    ...
    // Kiểm tra xem có thông tin của quản trị viên không và collection có phần tử nào không
    if($adminData && $adminData->isNotEmpty()) {
        // Lấy tên của quản trị viên từ phần tử đầu tiên trong collection
        $tenQTV = $adminData->first()->tenQTV;

        // Trả về view dashboard và truyền biến tenQTV vào view
        return view('dashboard', compact('tenQTV'));
    } else {
        // Nếu không có thông tin quản trị viên hoặc collection rỗng, chuyển hướng đến trang đăng nhập
