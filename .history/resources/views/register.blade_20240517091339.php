            ...
            <div id="formContent" class="formlogin" style="max-width: 900px; margin: auto;">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3 position-relative">
                        <label for="ngaySinhInput" class="form-label floating-label">Ngày sinh</label>
                        <input name="ngaySinh" type="date" id="ngaySinhInput" max="{{ date('Y-m-d') }}" class="fadeIn third form-control" value="{{ old('ngaySinh') }}" required>
                    </div>
                    <div class="col-md-6 mb-3">
                    ...
                    <div class="col-md-6 mb-3 position-relative">
                        <label for="ngayCapInput" class="form-label floating-label">Ngày cấp</label>
                        <input name="ngayCap" type="date" id="ngayCapInput" max="{{ date('Y-m-d') }}" class="fadeIn third form-control" value="{{ old('ngayCap') }}" required>
                    </div>
                    <div class="col-md-6 mb-3">
...
<script>
    ...
    inputs.forEach(input => {
        input.addEventListener('focus', () => {
            input.previousElementSibling.classList.add('floating-label-active');
        });
        input.addEventListener('blur', () => {
            if (input.value === '') {
                input.previousElementSibling.classList.remove('floating-label-active');
            }
        });
        if (input.value !== '') {
            input.previousElementSibling.classList.add('floating-label-active');
