        ...
        <div id="formContent" style="padding: 20px;">
            ...
            <div id="nomemForm">
                <form enctype="multipart/form-data" method="post" action="{{ url('admin/phongtros/'.$phong_id ?? '') }}">
                    ...
                    <!-- Kiểm tra xem $data có tồn tại không trước khi truy cập -->
                    <input type="text" name="rt_id" value="{{ $data->first()->id_loai_phong ?? '' }}">
                    <input type="text" name="tenPhong" value="{{ $data->first()->tenPhong ?? '' }}">
                    <!-- Kết thúc phần kiểm tra -->
                    <input type="text" name="mem_id" value="0">  
                    <!-- Hiển thị idKhachthue -->
                    <input type="text" name="cus_id" value="{{ session('data')->first()->id ?? '' }}"> 
                    <!-- Kết thúc phần hiển thị idKhachthue -->
            ...
            <div id="memForm" style="display: none; margin-top:-30px; transition:all linear 0.5s">
                <form enctype="multipart/form-data" method="post" action="{{ url('admin/phongtros/) }}">
                    @csrf
                    @method('put')
                    <!-- Kiểm tra xem $phong_id có tồn tại không trước khi truy cập -->
                    <input type="hidden" name="phong_id" value="{{ $phong_id ?? '' }}">
                    <!-- Kết thúc phần kiểm tra -->
