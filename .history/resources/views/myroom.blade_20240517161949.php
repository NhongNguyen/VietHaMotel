    ...
    @section('content')
        ...
        @if(isset($phongTro))
            ...
            @if(isset($hopDong))
                ...
                <td>
                                    <a href="{{ route('hopdong.view-pdf', ['id' => $hopDong->id]) }}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
                                    <!-- <a href="{{url('admin/hopdong/'.$d->id).'/edit'}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a> -->
                                    <a onclick="return confirm('Bạn đã chắc chắn muốn xóa dữ liệu này không?')" href="{{url('admin/hopdong/'.$hopDong->id).'/delete'}}" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
                                </td>
