        ...
        <div class="card mb-4">
            <div class="card-body" style="position:relative">
                @if ($phongtro->tinhTrang == "Còn trống")
                    <div style="position:absolute;right:0;top:0;transform:translate(17px,-15px);">
                        <button class="btn btn-success" style="border-radius:20px;">Còn phòng</button>
                    </div>
                @else
                    <div style="position:absolute;right:0;top:0;transform:translate(17px,-15px);">
                        <button disabled class="btn btn-danger" style="border-radius:20px;">Hết phòng</button>
                    </div>
                @endif
                ...
                <div style="text-align:center; margin-top:10px;">
                    @if ($phongtro->tinhTrang == "Còn trống")
                        <form action="{{ route('dang_ki_phong') }}" method="POST">
                            ...
                            <input type="submit" style="background-color:#C5A914;border-radius:20px;color:white" value="Đăng kí ngay"/>
                        </form>
                    @else
                        <form action="{{ route('dang_ki_phong') }}" method="POST">
                            @csrf
                            <input type="hidden" name="phong_id" value="{{ $phongtro->id }}">
                            <input type="submit" style="visibility:hidden;background-color:#C5A914;border-radius:20px;color:white" value="Đăng kí ngay" />
                        </form>
                    @endif
                </div>
            </div>
