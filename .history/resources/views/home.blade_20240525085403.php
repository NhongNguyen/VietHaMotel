...
document.getElementById('con-phong').addEventListener('click', function() {
    fetch('/con-phong')
        ...
        .then(data => {
            ...
            roomList.innerHTML = ''; 
            let cardsInRow = 0; // Biến đếm số lượng thẻ card đã được thêm vào trên hàng hiện tại
            data.forEach(room => {
                if (cardsInRow === 0) {
                    // Bắt đầu một hàng mới
                    roomList.innerHTML += '<div class="row">';
                }
                ...
                if (cardsInRow === 4) {
                    ...
                    roomList.innerHTML += '</div>';
                    cardsInRow = 0; // Đặt lại biến đếm cho hàng mới
                }
            });
        })
