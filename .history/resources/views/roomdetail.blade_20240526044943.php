        ...
        <div class="row">
            <div class="col-md-8">
              <a href="{{ asset('storage/'.$data->hinhAnhDaiDien) }}" data-lightbox="rgallery{{$data->id}}">
                  <img class="home-card-img-item" src="{{ asset('storage/'.$data->hinhAnhDaiDien) }}" alt="{{$data->tenPhong}}" />
              </a>
             
              <div class="room-images mt-4">
                  @foreach($data->hinhAnhPhongTros as $image)
              
                    <a href="{{ asset('storage/' . $image->srcHinhAnh) }}" data-lightbox="rgallery{{$data->}}">
                        <img class="img-thumbnail" src="{{ asset('storage/' . $image->srcHinhAnh) }}" style="width: 100px; height: auto;" alt="{{$data->tenPhong}}" />
                    </a>
                  @endforeach
              </div>
                <h2>{{ $data->tenPhong }}</h2>
