...
<div class="container my-4">
    <div class="pb-3 d-flex justify-content-between align-items-center" style="border-bottom: 4px solid #E6E6E6;">
        ...
        <div>
            <a href=""><button class="btn btn-success rounded-pill font-weight-bold" id="con-phong">Còn phòng</button></a>
            <div class="card mb-4 @if($phongtro->tinhTrang == 'Còn trống') con-phong @else het-phong @endif">
        </div>
    </div>
    <div class="row my-4">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const phongTros = document.querySelectorAll('.card.mb-4');

        // Lấy nút "Còn phòng"
        const conPhongButton = document.getElementById('con-phong');

        // Bắt sự kiện khi người dùng nhấn vào nút "Còn phòng"
        conPhongButton.addEventListener('click', function() {
            // Ẩn tất cả các phòng trước khi hiển thị chỉ các phòng còn trống
            phongTros.forEach(function(phongTro) {
                phongTro.style.display = 'none';
            });

            // Hiển thị chỉ các phòng có tình trạng là "Còn trống"
            const phongTroConPhong = document.querySelectorAll('.con-phong');
            phongTroConPhong.forEach(function(phongTro) {
                phongTro.style.display = 'block';
            });
        });
    });
</script>
