    ...
    <div class="container my-4" style="z-index:100">
        ...
        <div class="room-type-section">
            @foreach($loaiphongs as $loaiPhong)
                <a class="room-type-title" data-id="{{ $loaiPhong->id }}">{{ $loaiPhong->tenLoaiPhong }}</a>
            @endforeach
        </div>
...
document.addEventListener("DOMContentLoaded", function() {
    // Listen for click events on elements with the class 'room-type-title'
    var roomTypeTitles = document.querySelectorAll('.room-type-title');
    roomTypeTitles.forEach(function(roomTypeTitle) {
        roomTypeTitle.addEventListener('click', function() {
            // Get the ID of the room type from the data-id attribute
            var loaiPhongId = this.getAttribute('data-id');
            showRooms(loaiPhongId); // Call the showRooms function with the corresponding loaiPhongId
        });
    });
});

function showRooms(loaiPhongId) {
    // Send an Ajax request to fetch the rooms of that room type
    fetch('/get-rooms/' + loaiPhongId)
        .then(response => response.json())
        .then(data => {
            let roomList = document.getElementById('roomList');
            roomList.innerHTML = ''; // Clear the current room list

            // Create a new room list from the returned data
            data.forEach(room => {
                let roomHTML = `
                    <div class="col-sm-12 col-md-6 col-lg-4 col-xl-3 pt-4">
                ...
                `;
                roomList.innerHTML += roomHTML; // Add room information to the list
            });
        })
        .catch(error => console.error('Error:', error));
}

        // Lắng nghe sự kiện click trên các phần tử có class là 'room-type-title'
        var roomTypeTitles = document.querySelectorAll('.room-type-title');
        roomTypeTitles.forEach(function(roomTypeTitle) {
            roomTypeTitle.addEventListener('click', function() {
                // Lấy id của loại phòng từ thuộc tính data-id
                var loaiPhongId = this.getAttribute('data-id');
                showRooms(loaiPhongId); // Gọi hàm showRooms với loaiPhongId tương ứng
            });
        });
    });

    function showRooms(loaiPhongId) {
        // Gửi yêu cầu Ajax để lấy danh sách phòng của loại phòng đó
        fetch('/get-rooms/' + loaiPhongId)
            .then(response => response.json())
            .then(data => {
                let roomList = document.getElementById('roomList');
                roomList.innerHTML = ''; // Xóa bỏ danh sách phòng hiện tại

                // Tạo danh sách phòng mới từ dữ liệu được trả về
                data.forEach(room => {
                    let roomHTML = `
                        <div class="col-sm-12 col-md-6 col-lg-4 col-xl-3 pt-4">
                    ...
                    `;
                    roomList.innerHTML += roomHTML; // Thêm thông tin phòng vào danh sách
                });
            })
            .catch(error => console.error('Error:', error));
    }
