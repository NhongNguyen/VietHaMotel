    ...
    @section('content')
        ...
        <div class="container">
            ...
            @if(isset($phongTro))
                ...
                @if(isset($chiTietHoaDon) && is_array($chiTietHoaDon) && count($chiTietHoaDon) > 0)
                    @foreach ($chiTietHoaDon as $chiTiet)
                    <p>Số ngày thuê: {{ $chiTiet->soNgayThue }}</p>
                    <p>Đã thanh toán: {{ $chiTiet->daThanhToan }}</p>
                    <p>Thành tiền: {{ $chiTiet->thanhTien }}</p>
                    <!-- Hiển thị các thông tin khác về chi tiết hóa đơn -->
                    @endforeach
                @else
