        ...
        <div id="formContent" style=" padding:20px;">
            ...
            <div id="nomemForm">
                <form enctype="multipart/form-data" method="post" action="{{url('admin/phongtros/'.$phong_id ?? '')}}">
                    ...
                    <input type="text" name="rt_id" value="{{ $data->id_loai_phong }}">
                    <input type="text" name="tenPhong" value="{{ $data->tenPhong }}">
                    <input type="text" name="mem_id" value="{{ Auth::user()->id ?? '' }}">  
                    <!-- Hiển thị thông tin của khách thuê đăng nhập -->
                    <input type="text" name="id_khach_thue" value="{{ Auth::user()->id ?? '' }}"> 
                    <!-- Kết thúc phần hiển thị thông tin của khách thuê đăng nhập -->
            ...
            <div id="memForm" style="display: none; margin-top:-30px; transition:all linear 0.5s">
                <form enctype="multipart/form-data" method="post" action="{{url('admin/phongtros/'.$phong_id ?? '')}}">
                    ...
                    @method('put')
                    <input type="hidden" name="phong_id" value="{{ $phong_id ?? '' }}">
                    <div style="padding-top:20px; margin-bottom:-30px; color:black;"><h2>ĐĂNG KÝ THÀ
