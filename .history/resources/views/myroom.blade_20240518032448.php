    ...
    @section('content')
        ...
        @if(isset($thanhVien))
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <p>Họ tên khách: {{ $thanhVien->hoThanhVien }} {{ $thanhVien->tenThanhVien }}</p>
                    <p>Ngày sinh: {{ date('d-m-Y', strtotime($thanhVien->ngaySinh))}}</p>
                    <p>Giới tính: {{ $thanhVien->gioiTinh }}</p>
                    <p>Quê quán: {{ $thanhVien->queQuan }}</p>
                    <p>Nơi thường trú: {{ $thanhVien->noiThuongTru }}</p>
                    <p>Số CCCD: {{ $thanhVien->soCCCD }}</p>
                    <p>Ngày cấp: {{ date('d-m-Y', strtotime($thanhVien->ngayCap))}}</p>
                    <p>Nơi cấp: {{ $thanhVien->noiCap }}</p>
                    <p>Số điện thoại: {{ $thanhVien->SDT }}</p>
